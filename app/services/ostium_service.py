...
class OstiumService:
    ...
    async def place_order(
        ...
        try:
            ...
            execution_price = (
                limit_price
                if order_type == ORDER_TYPE.LIMIT and limit_price
                else latest_price
            )
    ...
    async def close_trade(
        ...
        try:
            ...
            for pos in positions:
                ...
                if (
                    pos_trade_id == str(pair_id)
                    or pos_pair_id == str(pair_id)
                    or nested_pair_id == str(pair_id)
                ):
                    position_exists = True
                    actual_pair_id = nested_pair_id or pos.get("pairId", pair_id)
                    actual_trade_index = pos.get("index", trade_index)
                    self.logger.info(
                        "Found position - pair.id: %s, index: %s",
                        actual_pair_id,
                        actual_trade_index,
                    )
                    break
            ...
            if not position_exists:
                return {
                    "success": False,
                    "error": f"Position with ID {pair_id} not found or already closed",
                }
        ...
        try:
            self.logger.info(
                "Closing trade - pair_id: %s, trade_index: %s, close_percentage: %s",
                pair_id,
                trade_index,
                close_percentage,
            )
            ...
            pair_id_uint16 = min(
                max(int(pair_id) if pair_id is not None else 0, 0), 65535
            )
            trade_index_uint8 = min(
                max(int(trade_index) if trade_index is not None else 0, 0), 255
            )
            close_percentage_uint16 = min(
                max(int(close_percentage) if close_percentage is not None else 100, 0),
                65535,
            )

            self.logger.info(
                "Converted arguments - pair_id: %s, trade_index: %s, close_percentage: %s",
                pair_id_uint16,
                trade_index_uint8,
            ...
            if result and result.get("order"):
                ...
                success_result = {
                    ...
                    "pnl": pnl,
                    "order_status": (
                        "Processed" if not order.get("isPending", True) else "Pending"
                    ),
                    "cancelled": order.get("isCancelled", False),
                }

                if order.get("isCancelled", False):
                    success_result["cancel_reason"] = order.get(
                        "cancelReason", "Unknown"
                    )
                    success_result["success"] = False
                    success_result["error"] = (
                        f"Order cancelled: {order.get('cancelReason', 'Unknown')}"
                    )
                else:
        ...
        try:
            ...
            if not trade.get("isOpen", True):
                profit_loss_percentage = float(
                    order.get("profitPercent") or trade.get("profitPercent") or 0
                )
            ...
            profit_loss_percentage = (
                (profit_loss / collateral) * 100 if collateral > 0 else 0
            )
